...
class ContentScriptManager {
    ...
    try {
      ...
      if (isAutomationWindow) {
        ...
        if (this.platform && this.platform !== "unknown") {
          ...
          // Set up auto-start timer in case background message is delayed
    ...
    indicator.innerHTML = `
      ...
      " onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 16px;" id="automation-status-icon">🤖</span>
          <div>
            <div style="font-weight: 700;" id="automation-status-text">AUTOMATION READY</div>
            <div style="font-size: 11px; opacity: 0.9;" id="automation-details">${this.platform?.toUpperCase()} • ${this.sessionId?.slice(
      -6
    )}</div>
          </div>
        </div>
  ...
  updateIndicatorStatus(status, icon = null) {
    const statusText = document.getElementById("automation-status-text");
    const statusIcon = document.getElementById("automation-status-icon");
    
    if (statusText) statusText.textContent = status;
    if (statusIcon && icon) statusIcon.textContent = icon;
  }
  ...
  async handleStartAutomation(request, sendResponse) {
    try {
      ...
      // Clear auto-start timer since we got the message
      ...
      this.log("🚀 Starting platform automation from background message");
      this.updateIndicatorStatus("AUTOMATION STARTING", "⏳");

      // Start the platform automation with the config
      await this.platformAutomation.start(this.config);

      this.updateIndicatorStatus("AUTOMATION ACTIVE", "🏃");
      ...
      this.updateIndicatorStatus("AUTOMATION ERROR", "❌");
      sendResponse({ success: false, error: error.message });
    }
  }
  ...
  async handlePauseAutomation(request, sendResponse) {
    if (this.platformAutomation && this.platformAutomation.pause) {
      ...
      this.updateIndicatorStatus("AUTOMATION PAUSED", "⏸️");
  ...
  async handleStopAutomation(request, sendResponse) {
    if (this.platformAutomation && this.platformAutomation.stop) {
      ...
      this.updateIndicatorStatus("AUTOMATION STOPPED", "⏹️");
  ...
  setupAutoStartTimer() {
    // Set up a timer to auto-start automation if we don't get the message from background
    this.startupTimer = setTimeout(async () => {
      if (this.platformAutomation && !this.platformAutomation.isRunning) {
        this.log("⏰ Auto-starting automation (background message timeout)");
        try {
  ...
  showAutomationStatus() {
    ...
    const status = this.platformAutomation?.getStatus?.() || "Unknown";
    const isRunning = this.platformAutomation?.isRunning || false;
    ...
    modal.innerHTML = `
      <div style="background: white; padding: 24px; border-radius: 12px; max-width: 400px; box-shadow: 0 8px 32px rgba(0,0,0,0.3);">
        ...
        <p><strong>Status:</strong> ${isRunning ? 'Active' : 'Inactive'}</p>
        <p><strong>Automation State:</strong> ${JSON.stringify(status)}</p>
